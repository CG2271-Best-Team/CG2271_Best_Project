...
void Init_PWN(void) 
{
	...
	PORTB->PCR[Left_Motor_Pin_F] &= ~PORT_PCR_MUX_MASK;
  PORTB->PCR[Left_Motor_Pin_F] |= PORT_PCR_MUX(3);
	PORTB->PCR[Right_Motor_Pin_B] &= ~PORT_PCR_MUX_MASK;
	PORTB->PCR[Right_Motor_Pin_B] |= PORT_PCR_MUX(3);
	PORTB->PCR[Left_Motor_Pin_B] &= ~PORT_PCR_MUX_MASK;
  PORTB->PCR[Left_Motor_Pin_B] |= PORT_PCR_MUX(3);
	...
	TPM2_C1V = (FREQUENCY / 2); // 50 percent speed
}
...
void playNote(float note, int duration) {
    if (note != 0) {
			...
			TPM2_MOD = tone;
			TPM2_C1V = (tone / 2); // 50 percent speed
    } else {
			TPM2_MOD = 0;
			TPM2_C1V = 0;
		}
...
void playHappyBirthday() {
    float notes[] = {NOTE_G4, NOTE_G4, NOTE_A4, NOTE_G4, NOTE_C5, NOTE_B4, NOTE_G4, 0,
...
void Motor_Control(int motion) { //left 0, right 1, forward 2, back 3, rest 4 
